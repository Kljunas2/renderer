...
def draw_face(canvas, face):
	...
	coords = tuple(map(lambda x: 50 * x + 250,
		[loop[0][0], loop[0][1], loop[1][0], loop[1][1], loop[2][0], loop[2][1]]))
	shade = 275 - int(normal * 240)
	fill = _from_rgb((shade, shade, shade))
	canvas.create_polygon(coords, fill=fill)


with open("shapes/cube.stl", "r") as f:
	solid = parser.parse(f.read())
	print("Parmath.sing done.")
...
while True:
	w.delete("all")
	sleep(0.01)
	camera1.phi = -speed * clock() + math.pi
	camera1.x = math.sin(speed * clock()) * 40
	camera1.y = -math.cos(speed * clock()) * 40
	camera1.update()
